...
import { useRevalidator, useSearchParams } from "@remix-run/node";
...
const Shipping: React.FC<ShippingProps> = ({ cart, availableShippingMethods }) => {
  ...
  useEffect(() => {
    ...
    if (_shippingMethods?.length) {
      const promises = _shippingMethods
        ...
        .map(async (sm) => {
          const request = await fetch("/fulfillment/calculate-price-for-shipping", {
            method: "POST",
            body: JSON.stringify({
              cart_id: cart.id,
              option_id: sm.id,
  ...
  const handleEdit = () => {
    setSearchParams({ step: "delivery" }, { preventScrollReset: true });
  };
  ...
  const handleSetShippingMethod = async (id: string, variant: "shipping" | "pickup") => {
    ...
    const request = await fetch("/cart/set-shipping-method", {
      method: "POST",
      body: JSON.stringify({
        cart_id: cart.id,
        ...
        <>
          <div className="grid">
            ...
            <div data-testid="delivery-options-container">
              <div className="pb-8 md:pt-0 pt-2">
                ...
                <RadioGroup value={shippingMethodId} onChange={(v) => handleSetShippingMethod(v, "shipping")}>
          ...
          {showPickupOptions === PICKUP_OPTION_ON && (
            <div className="grid">
              ...
              <div data-testid="delivery-options-container">
                <div className="pb-8 md:pt-0 pt-2">
                  <RadioGroup value={shippingMethodId} onChange={(v) => handleSetShippingMethod(v, "pickup")}>
